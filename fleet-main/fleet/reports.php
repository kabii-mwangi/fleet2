...
// Get filtered fuel logs
try {
    $sql = "
        SELECT fl.*, v.registration_number, v.make, v.model, vc.name as category_name, e.name as employee_name
        FROM fuel_logs fl 
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (empty($reportLogs)): ?>
                            <tr>
                                <td colspan="8" class="no-data">No fuel logs found for the selected period</td>
                            </tr>
                        <?php else: ?>
